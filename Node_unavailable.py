#!/usr/bin/env python3
"""
MICROCORE (MCX) COMPLETE NODE v21.4 — MAINNET READY
...
*** FIXED IN v21.4 ***
- ALL fees paid in MCX (no USD conversion)
- Removed MCX_PRICE_USD and DUCO_PRICE_USD
- Fee = 0.3% of swap amount in MCX directly
- All payments unified in MCX
- DEX integration ready (THORChain/LI.FI)
- Node rewards tracking by wallet

Usage:
  # Set environment variables first (Stripe only)
...
# MCX_PRICE_USD and DUCO_PRICE_USD REMOVED — all fees in MCX
...
# Fee = 0.3% of the swap amount in MCX directly (no USD conversion)
...
class DEX:
    ...
    def _calc_fee_mcx(self, amount: float) -> int:
        """
        Calculate swap fee in MCX (0.3% of swap amount in MCX)
        NO USD CONVERSION — all fees in MCX
        """
    ...
    async def get_lifi_quote(self, from_token, to_token, amount):
        try:
            # LI.FI quote — fee in MCX based on swap amount
            out = amount * 0.997
            fee = self._calc_fee_mcx(amount)
    ...
    async def get_thorchain_quote(self, from_token, to_token, amount):
        try:
            # THORChain quote — fee in MCX based on swap amount
            out = amount * 0.995
            fee = self._calc_fee_mcx(amount)
    ...
    async def execute_swap(self, wallet, from_token, to_token, amount, fee):
        ...
        # Fee is paid in MCX (0.3% of swap amount in MCX)
    ...
    def buy_mcx(self, wallet, usd_amount, payment_method="card"):
        ...
        mcx_amount = int(usd_amount / 0.01)  # Fixed rate for fiat purchases
        ...
        self.net.conn.commit()
        return True, {"mcx": mcx_amount, "usd": usd_amount}
...
class MicroCoreNetwork:
    ...
    def process_duco_purchase(self, wallet: str, username: str, txid: str, tx_time: float, duco_amount: float) -> dict:
        ...
        # DUCO to MCX conversion: 1 MCX = 1000 DUCO (fixed rate)
    ...
    def create_stripe_payment_intent(self, wallet, username, usd_amount):
        try:
            ...
            mcx_amount = int(usd_amount / 0.01)  # Fixed rate: 1 MCX = $0.01
